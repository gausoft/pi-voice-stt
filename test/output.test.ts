...
test("formatTranscriptForPrompt trims and appends a trailing space by default", () => {
  assert.equal(formatTranscriptForPrompt("  hello world  ", { appendTrailingSpace: true }), "hello world ");
});

test("formatTranscriptForPrompt can omit trailing space", () => {
  assert.equal(formatTranscriptForPrompt("  hello world  ", { appendTrailingSpace: false }), "hello world");
});

test("formatTranscriptForPrompt returns empty text for empty transcripts", () => {
  assert.equal(formatTranscriptForPrompt("   ", { appendTrailingSpace: true }), "");
});
